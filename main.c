...
void list(char * path, user_t user) {
    ...
    if (path == NULL)
        ...
        path = pathcheck(path, user);
    
    pdir = opendir(path);

    if (pdir == NULL) {
        fprintf(stderr, RED "ERROR " RESET" mysh: unable to open directory\n");
        return;
    }
...
int make(char * filename, user_t user) {
    FILE * fp;
    pathcheck(filename, user);

    // Check if file already exists first
    if (isfile(filename))
    {
        fprintf(stderr, RED "ERROR " RESET "mysh: file already exists.\n");
        return 0;
    }

    // Just in case there's anything wrong with opening the file to begin with.
...
int repeat(char ** args, int * pidarr, user_t user) {
    ...
    if (!(num = strtol(args[1], &ptr, DECIMAL))) {
...
void builtin(char * input, char ** argv, user_t user, pid_t * pid, 
    ...
    if (!strcmp(argv[0], "whereami"))
        ...
        char ** ptr = NULL;
        if (argv[1] == NULL) {
            fprintf(stderr, RED "ERROR " RESET "mysh : PID required for dalek\n");
            return;
        }
    ...
    {
        repeat(argv, pid, user);
    }

    else if (!strcmp(argv[0], "background")) {
        tmp++;
        background(tmp, pid, user);
        tmp = NULL;
    }
